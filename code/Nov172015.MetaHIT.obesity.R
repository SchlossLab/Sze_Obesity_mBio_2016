...
BFRatio = Bacter/Firm
alpha.diversity <- cbind(alpha.diversity, BFRatio)

median(BFRatio) # 0.3008129
alpha.diversity <- within(alpha.diversity, {BFRatio.cat = ifelse(BFRatio <= median(BFRatio), "less", "higher")})

BFRatio.cat <- alpha.diversity$BFRatio.cat
test4 <- cbind(BFRatio.cat, bmi.cat)
test4 <- test4[order(BFRatio.cat), ]
table(test4[c(1:34), 2]) #higher group
table(test4[c(35:68), 2]) #lower group
#Group1 (Higher than median), obese = 15 and non-obese = 19
#Group2 (Lower than median), obese = 17 and non-obese = 17
