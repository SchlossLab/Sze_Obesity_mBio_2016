...
So now the biggest challenge that I have found so far is getting the data into a distance matrix not based on the closed-reference approach.  The file is just too big for cluster.split so we have had to use the closed-reference approach for this data set.
