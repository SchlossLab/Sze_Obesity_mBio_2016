...
BFNorm <- (log(BFratio) - log(0.3970972))/log(0.4232106)

ZscoreBacter <- scale(bacter)
ZScoreFirm <- scale(firm)
ZBF <- scale(BFratio)
escobar.BandF.scales <- cbind(rownames(phyla.table.rel.abund), ZscoreBacter, ZScoreFirm, ZBF)
colnames(escobar.BandF.scales) <- c("Sample", "Zbacter", "Zfirm", "ZBF")
write.csv(escobar.BandF.scales, "escobar.BF.phyla.stand.csv")
