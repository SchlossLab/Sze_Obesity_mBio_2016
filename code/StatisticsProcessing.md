...
test2 <- test[keep, ]
good.metadata <- test2
test3 <- test2[!duplicated(test2$submitted_subject_id_s), ]
keep <- rownames(test3)
microbiome <- shared.data[keep, ]
metadata <- test3

rm(good.metadata, shared.data, keep, test, test2, test3)
...
anova(lm(H ~ obese)) #P-value=0.2034
anova(lm(S ~ obese)) #P-value=0.03516
anova(lm(J ~ obese)) #P-value=0.5369
...
anova(lm(bacter ~ obese)) #P-value=0.9582
anova(lm(firm ~ obese)) #P-value=0.2654
anova(lm(BFratio ~ obese)) #P-value=0.9899
...
set.seed(3)
adonis(microbiome ~ obese, permutations=1000) 
#PERMANOVA=0.5734, pseudo-F=0.95626
...
test3 <- cbind(H.cat, S.cat, J.cat, bmi.cat)
test3 <- test3[order(H.cat), ]
table(test3[c(1:103), 4])
table(test3[c(104:207), 4])
...
test4 <- cbind(BFRatio.cat, obese)
test4 <- test4[order(BFRatio.cat), ]
table(test4[c(1:103), 2]) #higher group
table(test4[c(104:207), 2]) #lower group
...
set.seed(3)
fit <- AUCRF(obese ~ ., data=testset, ntree=1000, nodesize=20)
summary(fit) # list of 42 Measures, AUCopt = 0.6365534
plot(fit)
```
...
#text(3.2, 3.10, "Relative Risk [95% CI]", pos=2, cex=0.8)#
  #large scale (Plot in R studio at full)                 #
                                                          #
text(5.5, 10.3, "Relative Risk [95% CI]", pos=2, cex=0.8)  #
...
m = 8
T = -2*log(0.608)-2*log(0.214)-2*log(0.236)-2*log(0.439)-2*log(0.575)-2*log(0.376)-2*log(0.025)-2*log(0.64)-2*log(0.332)
T.pval = 1-pchisq(T,m) #P-value = 0.00464

##Alternatively can use built in package (I trust this more)
anova.rma(res)
# QM = 6.0488, p-value = 0.0139
```
<br>
...
##Calculate combined P-value
m = 8
T = -2*log(0.864)-2*log(1.00)-2*log(0.089)-2*log(0.121)-2*log(0.452)-2*log(0.111)-2*log(0.53)-2*log(0.64)-2*log(0.894)
T.pval = 1-pchisq(T,m) #P-value = 0.02338081

##Alternatively can use built in package (I trust this more)
anova.rma(res)
  # QM = 4.7318, p-value = 0.0296
```
...
t.test(H.Zscore ~ obese, data=group.data.test2) #P-value = 0.00493
table(group.data.test2$obese) # No = 1099 | Yes = 443
p1 <- ggplot(group.data.test2, aes(obese, H.Zscore)) + geom_boxplot()

#A more in-depth look, breaking down the groups
anova(lm(H.Zscore ~ BMI.class, data=group.data.test1)) #P-value = 0.03826
a2 <- aov(H.Zscore ~ BMI.class, data=group.data.test1)
posthoc <- TukeyHSD(x=a2, 'BMI.class', conf.level=0.95)
  # Obese-Lean (p-value=0.107), Over-Norm (p-value=0.872), Over-Obese (p-value=0.0394479)

table(group.data.test1$BMI.class) 
  # Lean = 509 | Overweight = 365 | Obese = 461
ddply(group.data.test1, ~BMI.class, summarise, mean=mean(H.Zscore), sd=sd(H.Zscore))
  # Lean = 0.027 +\- 0.993| Overweight = 0.059 +/- 0.982 | Obese = -0.111 +\- 1.016
...
summary(lm(HZScore ~ bmi, data=data1)) #P-value 0.1272, R2= 0.001959
...
amish.norm.BF <- am.BF.corr/0.7918527
hist(amish.norm.BF)
...
hist(COMBO.norm.BF)
#twinsUK SD = 0.9802656
keeptwinsUK <- which(data2$dataset == "twinsUK")
twinsUKdata <- data2[keeptwinsUK, ]
twinsUK.norm.BF <- twinsUKdata$BF.ratio.corr/0.9802656
hist(twinsUK.norm.BF)

#data1 order Escobar, GLNE, CCHC, HMP, Korean, MetaHit
BFScore <- c(Escobar.norm.BF, GLNE.norm.BF, CCHC.norm.BF, HMP.norm.BF, Korean.norm.BF, metahit.norm.BF, COMBO.norm.BF, twinsUK.norm.BF)
data2 <- cbind(BFScore, data2)

amish.data <- cbind(amish.norm.BF, amish.data)
turn.data <- cbind(turn.norm.BF, turn.data)
rm(Escobar.norm.BF, GLNE.norm.BF, CCHC.norm.BF, HMP.norm.BF, Korean.norm.BF, metahit.norm.BF, amish.norm.BF, turn.norm.BF, COMBO.norm.BF, twinsUK.norm.BF)
rm(keepmetahit, keepCCHC, keepescobar, keepGLNE, keepHMP, keepKorean)
...
posthoc <- TukeyHSD(x=a2, 'BMI.class', conf.level=0.95)
  # Obese-Norm (p-value=0.016) | Over-Norm (p-value=0.017), Over-Obese (p-value=0.979)
...
ddply(group.data.test, ~BMI.class, summarise, mean=mean(BF.ratio.corr), sd=sd(BF.ratio.corr))
  # Lean = -0.392 +\- 1.319 | Overweight = -0.695 +/- 1.705 | Obese = -0.718 +\- 2.180
bf2 <- ggplot(group.data.test, aes(BMI.class, BF.ratio.corr)) + geom_boxplot()


summary(lm(BFScore ~ bmi, data=data2)) #P-value 0.06589, R2= 0.002847
...
#A more in-depth look, breaking down the groups
anova(lm(Zbacter ~ BMI.class, data=group.data.test)) #P-value = 0.03564
a2 <- aov(Zbacter ~ BMI.class, data=group.data.test)
posthoc <- TukeyHSD(x=a2, 'BMI.class', conf.level=0.95)
  # Obese-Norm (p-value=0.999) | Over-Norm (p-value=0.054) | Over-Obese (p-value=0.085)
table(group.data.test$BMI.class)
  # Lean = 509 | Overweight = 461 | Obese = 365
ddply(group.data.test, ~BMI.class, summarise, mean=mean(Zbacter), sd=sd(Zbacter))
  # Lean = 0.051 +/- 0.970 | Overweight = -0.097 +/- 1.000 | Obese = 0.051 +/- 1.02

bacter2 <- ggplot(group.data.test, aes(BMI.class, Zbacter)) + geom_boxplot()


summary(lm(Zbacter ~ bmi, data=data2)) #P-value 0.8789, R2= 1.957x10-5
...
###################################Firmicutes#######################################
#Look at Obese versus non-obese
t.test(Zfirm ~ obese, data=group.data.test2) #P-value = 0.8582
table(group.data.test2$obese)
  # No = 1099 | Yes = 443
...
posthoc <- TukeyHSD(x=a2, 'BMI.class', conf.level=0.95)
  # Obese-Norm (p-value=0.812), Over-Norm (p-value=0.017), Over-Obese (p-value=0.134)
table(group.data.test$BMI.class)
  # Lean = 509 | Overweight = 461 | Obese = 365
ddply(group.data.test, ~BMI.class, summarise, mean=mean(Zfirm), sd=sd(Zfirm))
  # Lean = -0.072 +/- 0.965 | Overweight = 0.104 +/- 1.008 | Obese = -0.030 +/- 1.020

firm2 <- ggplot(group.data.test, aes(BMI.class, Zfirm)) + geom_boxplot()


summary(lm(Zfirm ~ bmi, data=data2)) #P-value 0.7604, R2= 7.835x10-5
...
#Look at Obese versus non-obese
t.test(zbf ~ obese, data=group.data.test2) #P-value = 0.4516
table(group.data.test2$obese)
  # No = 1099 | Yes = 443
...
posthoc <- TukeyHSD(x=a2, 'BMI.class', conf.level=0.95)
  # Obese-Norm (p-value=0.979) | Over-Norm (p-value=0.434) | Over-Obese (p-value=0.383)
table(group.data.test$BMI.class)
  # Lean = 509 | Overweight = 461 | Obese = 365
ddply(group.data.test, ~BMI.class, summarise, mean=mean(zbf), sd=sd(zbf))
  # Lean = 0.0024 +/- 0.979 | Overweight = -0.055 +/- 0.988 | Obese = 0.037 +/- 1.033
zbf2 <- ggplot(group.data.test, aes(BMI.class, zbf)) + geom_boxplot()


summary(lm(ZBF ~ bmi, data=data2)) #P-value 0.8311, R2= 3.836x10-5
