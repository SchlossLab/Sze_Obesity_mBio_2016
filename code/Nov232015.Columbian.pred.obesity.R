...
summary(fit2) # list of 7 Measures, AUCopt = 0.915
plot(fit2)

set.seed(3)
fit3 <- AUCRF(obese ~ ., data=testset, ntree=600, nodesize=20)
summary(fit3) # list of 7 Measures, AUCopt = 0.915
plot(fit3)

set.seed(3)
fit4 <- AUCRF(obese ~ ., data=testset, ntree=400, nodesize=20)
summary(fit4) # list of 5 Measures, AUCopt = 0.9
plot(fit4)
rm(testset)
...
testset <- Filter(function(x)(length(unique(x))>5), columbian.microb)
testset <- cbind(testset, H, S, J, phyla.table.rel.abund)
testset <- cbind(edit.metadata2$body_mass_index_s, testset)
...
#Variables OTU00015, OTU00016, OTU00071, OTU00132, OTU00135, OTU00150, OTU00161
#Variables cont. OTU00260, OTU00370, Verrucomicrobia
test <- randomForest(testset[,getSelectedAttributes(bmi.bortuaRegress)], testset$bmi)
# No. variables at each split = 3, No. trees = 500, % Var.explained = 47.86, mean of squared residuals = 10.27352
