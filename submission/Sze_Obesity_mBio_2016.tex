...
used AMOVA to test for a significant differences between the structure
of non-obese and obese individuals. The Escobar, Goodrich, and Turnbaugh
datasets indicated a significant difference in community structure (all
P\textless{}0.05). Because it was not possible to ascertain the
directionality of the difference in community structure nor perform a
...
to results that cannot be reproduced (23). Future microbiome studies
...
3 studies (7, 10, 22) and a metagenomic study (24) and the Walters study
used 16S rRNA gene sequence data from 5 studies (10, 15, 17, 22, 25);
two studies were included in both analyses (10, 22). Our analysis
included 4 of these studies (10, 15, 17, 22) and excluded 3 of the
studies because they were too small (7), only utilized metagenomic data
(24), or used short single read Illumina HiSeq data that has a high
error rate making it untractable for \emph{de novo} OTU clustering (25).
...
(26). A detailed description of our selection process and the exact
...
10 studies, 3 were from identified from our PubMed search (6, 15, 18), 5
were originally identified from the 9 studies that did not explicitly
investigate obesity but included BMI data (14, 16, 17, 21, 22), and two
datasets were used (19, 20) because these publications did not
...
In total seven studies used 454 (6, 14, 15, 17, 18, 20, 22) and three
studies used Illumina sequencing (16, 19, 21). All of these studies used
amplification-based 16S rRNA gene sequencing. Among the studies that
sequenced the 16S rRNA gene, the researchers targeted the V1-V2 (17),
V1-V3 (14, 15, 18), V3-V5 (20, 22), V4 {[}(16); (19); {]}, and V3-4 (21)
...
(6, 22). We processed the 16S rRNA gene sequence data using a
...
community structure from non-obese and obese individuals using analysis
of molecular variance (AMOVA) with Bray-Curtis distance matrices (32).
This analysis was performed using the vegan (v.2.3-5) R package. For
both analyses, the datasets were rarefied (N=1000) so that each study
within a study had the same number of sequences. Second, for each study
we partitioned the subjects into a low or high group depending on
whether their alpha diversity metrics were below or above the median
value for the study. The relative risk (RR) was then calculated as the
ratio of the number of obese individuals in the low group to the number
of obese individuals in the high group. We then performed a Fisher
exact-test to investigate whether the RR was significantly different
from 1.0 within each study and across all of the studies using the epiR
(0.9-77) and metafor (1.9-8) packages. Third, we used the AUCRF (1.1) R
package to generate Random Forest models. For each study we developed
models using either OTUs or genus-level phylotypes. The quality of each
model was assessed by measuring the area under the curve (AUC) of the
Receive Operating Characteristic (ROC) using ten-fold cross validation.
Because the genus-level phylotype models were developed using a common
reference, it was possible to use one study's model (i.e.~the training
set) to classify the samples from the other studies (i.e.~the testing
sets). The optimum threshold for the training set was set as the
probability threshold that had the highest combined sensitivity and
specificity. This threshold was then used to calculate the accuracy of
the model applied to the test studies. To generate Receiver Operator
...
16. \textbf{Goodrich JK}, \textbf{Waters JL}, \textbf{Poole AC},
...
17. \textbf{Wu GD}, \textbf{Chen J}, \textbf{Hoffmann C},
...
18. \textbf{Escobar JS}, \textbf{Klotz B}, \textbf{Valdes BE},
...
19. \textbf{Baxter NT}, \textbf{Ruffin MT}, \textbf{Rogers MAM},
...
21. \textbf{Zeevi D}, \textbf{Korem T}, \textbf{Zmora N},
...
22. \textbf{Human Microbiome Project Consortium}. 2012. Structure,
...
23. \textbf{Ioannidis JPA}. 2005. Why most published research findings
...
24. \textbf{Qin J}, \textbf{Li R}, \textbf{Raes J}, \textbf{Arumugam M},
...
25. \textbf{Yatsunenko T}, \textbf{Rey FE}, \textbf{Manary MJ},
...
26. \textbf{Moher D}, \textbf{Liberati A}, \textbf{Tetzlaff J},
